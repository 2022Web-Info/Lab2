...
def mul(W_r: torch.Tensor, entity_embed: torch.Tensor):
    """
        TODO: 将实体投影到关系空间，返回其表征
        W_r:    (batch_size, embed_dim, relation_dim)   # batch_size个对应位置的变化矩阵
        h:      (batch_size, embed_dim) # batch_size个实体
    """
...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        W_r = self.trans_M[r]  # (kg_batch_size, embed_dim, relation_dim), 每一个关系对应的关系空间的映射
        ...
        # TODO:1. 计算头实体，尾实体和负采样的尾实体在对应关系空间中的投影嵌入

        r_mul_h = mul(
            W_r, h_embed
        )  # (batch_size, relation_dim)                                                                     # (kg_batch_size, relation_dim)
        r_mul_pos_t = mul(
            W_r, pos_t_embed
        )  # (batch_size, relation_dim)                                                                      # (kg_batch_size, relation_dim)
        r_mul_neg_t = mul(
        ...
        # TODO:2. 对关系嵌入，头实体嵌入，尾实体嵌入，负采样的尾实体嵌入进行L2范数归一化
        r_embed = r_embed / torch.norm(r_embed, dim=1,
                                       keepdim=True)  # (batch_size, relation_dim)
        ...
        # TODO:3. 分别计算正样本三元组 (h_embed, r_embed, pos_t_embed) 和负样本三元组 (h_embed, r_embed, neg_t_embed) 的得分
        pos_score = torch.norm(
            r_mul_h + r_embed - r_mul_pos_t, dim=1
        )**2  # (batch_size)                                                                     # (kg_batch_size)
        neg_score = torch.sum(r_mul_h * r_embed - r_mul_neg_t,
                              dim=1)**2  # (kg_batch_size)

        # TODO:4. 使用 BPR Loss 进行优化，尽可能使负样本的得分大于正样本的得分
        kg_loss = (-1.0) * F.logsigmoid(neg_score - pos_score)  # 这里跟一般的bpr相反，因为要使正样本得分小
        kg_loss = torch.mean(kg_loss)

        l2_loss = _L2_loss_mean(r_mul_h) + _L2_loss_mean(r_embed) + _L2_loss_mean(
            r_mul_pos_t) + _L2_loss_mean(r_mul_neg_t)
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        # TODO:5. 对关系嵌入，头实体嵌入，尾实体嵌入，负采样的尾实体嵌入进行L2范数归一化
        ...
        # TODO:6. 分别计算正样本三元组 (h_embed, r_embed, pos_t_embed) 和负样本三元组 (h_embed, r_embed, neg_t_embed) 的得分
        pos_score = torch.norm(h_embed + r_embed - pos_t_embed,
                               dim=1)**2  # (kg_batch_size)
        neg_score = torch.norm(h_embed + r_embed - neg_t_embed,
                               dim=1)**2  # (kg_batch_size)

        # TODO:7. 使用 BPR Loss 进行优化，尽可能使负样本的得分大于正样本的得分
        kg_loss = (-1.0) * F.logsigmoid(neg_score - pos_score)
        kg_loss = torch.mean(kg_loss)

        l2_loss = _L2_loss_mean(h_embed) + _L2_loss_mean(r_embed) + _L2_loss_mean(
            pos_t_embed) + _L2_loss_mean(neg_t_embed)
        loss = kg_loss + self.kg_l2loss_lambda * l2_loss
        return loss
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        # item_neg_cf_embed = torch.concat((item_neg_embed,item_neg_cf_embed),dim=1)
    ...
    def calc_score(self, user_ids, item_ids):
        ...
        # TODO:9. 为 物品嵌入 注入 实体嵌入的语义信息
        # 相加
        item_cf_embed = item_embed + item_kg_embed  # (n_items, embed_dim)
        # # 逐元素相乘  # TODO: 分别跑这三种拼接方式，注意和8处保持一致
        # item_cf_embed = item_embed * item_kg_embed
        # # 拼接
        # item_kg_embed = torch.concat((item_embed,item_kg_embed),dim=1)

        cf_score = torch.matmul(user_embed,
                                item_cf_embed.transpose(0, 1))  # (n_users, n_items)
