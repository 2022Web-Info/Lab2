...
class Aggregator(nn.Module):
    ...
    def forward(self, ego_embeddings, A_in):
        ...
        side_embeddings = torch.mm(A_in, ego_embeddings)  # (n_users + n_entities, embed_dim)
        ...
        if self.aggregator_type == 'gcn':
            ...
            embeddings = torch.cat([ego_embeddings, side_embeddings], dim=1)  # (n_users + n_entities, embed_dim * 2)
            embeddings = self.activation(self.linear(embeddings))  # (n_users + n_entities, embed_dim)
...
class GNN_based(nn.Module):
    ...
    def calc_cf_embeddings(self):
        ...
        for idx, layer in enumerate(self.aggregator_layers):
            ...
            embed_nor = F.normalize(ego_embed, p=2, dim=1)
            all_embed.append(embed_nor)
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        neg_score = torch.norm(h_embed + r_embed - neg_t_embed, p=1, dim=-1,
                               keepdim=True)  # (kg_batch_size)                                                                     # (kg_batch_size)
